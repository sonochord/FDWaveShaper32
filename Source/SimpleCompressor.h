...
// I have added in a high pass filter for the sideChainBuffer using JUCE filter library:: Fergal Davis
#pragma once
...
public:
    ...
    void prepare (const ProcessSpec& specs) override
    {
        ...
        dsp::ProcessSpec filterSpec;
        filterSpec.sampleRate = specs.sampleRate;
        filterSpec.maximumBlockSize = specs.maximumBlockSize;
        filterSpec.numChannels = 1;

        highPassFilter.prepare(filterSpec);
        highPassFilter.coefficients = dsp::IIR::Coefficients<float>::makeHighPass(specs.sampleRate, 120.0f); // 120 Hz high-pass filter
    };
    ...
    void process (const ProcessContextReplacing<float>& context) override
    {
        ...
        // Apply high-pass filter to the sidechain signal
        dsp::AudioBlock<float> sideChainBlock(sideChainBuffer);
        dsp::ProcessContextReplacing<float> sideChainContext(sideChainBlock);
        highPassFilter.process(sideChainContext);
...
private:
    ...
    dsp::IIR::Filter<float> highPassFilter; // High-pass filter member
};
